...
privileged aspect TextController_Roo_Controller {
    ...
    @Autowired
    TextService TextController.textService;
    ...
    public String TextController.show(@PathVariable("id") Long id, Model uiModel) {
        uiModel.addAttribute("text", textService.findText(id));
        uiModel.addAttribute("itemId", id);
        return "texts/show";
    }
    
    @RequestMapping(produces = "text/html")
    public String TextController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("texts", Text.findTextEntries(firstResult, sizeNo, sortFieldName, sortOrder));
            float nrOfPages = (float) textService.countAllTexts() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("texts", Text.findAllTexts(sortFieldName, sortOrder));
        }
        return "texts/list";
    }
    ...
    public String TextController.updateForm(@PathVariable("id") Long id, Model uiModel) {
        populateEditForm(uiModel, textService.findText(id));
        return "texts/update";
    }
